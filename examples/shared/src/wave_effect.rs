...
impl WaveInterference {
    ...
    fn calc_wave_amplitude_with_data(waves: &[Wave], elapsed: f32, pos: (f32, f32)) -> f32 {
...
impl Shader for WaveInterference {
    ...
    fn process(&mut self, duration: Duration, buf: &mut Buffer, area: Rect) -> Option<Duration> {
        ...
        for (pos, cell) in cell_iter {
            ...
            let normalized = Self::calc_wave_amplitude_with_data(&waves, elapsed, pos);
            assert!(
            ...
            let hue = (normalized * 360.0 + (elapsed * hue_shift_speed) + 1.4 * pos.0
                - 3.2 * pos.1 * elapsed_cos)
                % 360.0;
