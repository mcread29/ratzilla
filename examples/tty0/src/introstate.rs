...
use ratzilla::ratatui::widgets::{Block, Gauge, Paragraph, Wrap};
...
impl<'a> LineByLine<'a> {
    ...
    /// Returns the percentage of the lines that have been displayed.
    ...
    /// Returns the up to current line of this [`LineByLine`].
    ...
    pub fn update(&mut self, _elapsed: Duration) {
        ...
        let list: Vec<ListItem> = self
            ...
            .collect();
        self.lines.state.select(Some(list.len() - 1));
    }
}
