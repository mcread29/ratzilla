import { ChangeEvent, DragEvent, MouseEvent, ReactNode, useEffect, useLayoutEffect, useMemo, useRef, useState } from "react";
import {
...
type ShapeInteractionMode = "add" | "move" | "delete";
...
const DEFAULT_TIMELINE_VIEWPORT_WIDTH = 960;
const MAX_VISIBLE_MEASURES_AT_MAX_ZOOM = 16;
...
const SHAPE_EDITOR_HANDLE_INSET = 6;
const LFO_POINT_SNAP = 0.025;
const CLIP_LENGTH_BAR_OPTIONS = [
  { label: "4 bars", bars: 4 },
  { label: "3 bars", bars: 3 },
  { label: "2 bars", bars: 2 },
  { label: "1 bar", bars: 1 },
...
export default function App() {
  ...
  const dirty = useMemo(
    () => (loaded ? draftSnapshot !== loaded.savedSnapshot : false),
    [draftSnapshot, loaded],
  );
  ...
  function commitSelectedClipShape(points: LfoPoint[]) {
    ...
    updateDraft((current) => {
      ...
      clip.source.shape.points = points;
    ...
    <div className="app-shell">
      <audio
        ...
        onLoadedMetadata={() => {
          ...
          setAudioDuration(typeof duration === "number" && Number.isFinite(duration) ? duration : null);
        }}
        ...
        <section className="left-column">
            ...
            <section className="panel arrangement-panel">
              <div className="inline-actions arrangement-toolbar">
                ...
                <div className="arrangement-toolbar-actions">
                  ...
                  <button
                    className="icon-button"
                    onClick={copySelectedPlacements}
                    disabled={!selectedPlacementIndices.length}
                    type="button"
                    title={`Copy placement${selectedPlacementIndices.length === 1 ? "" : "s"}`}
                    aria-label={`Copy placement${selectedPlacementIndices.length === 1 ? "" : "s"}`}
                  >
                  ...
                  <button
                    ...
                    disabled={!selectedPlacementIndices.length}
                    type="button"
                    title={`Delete placement${selectedPlacementIndices.length === 1 ? "" : "s"}`}
                    aria-label={`Delete placement${selectedPlacementIndices.length === 1 ? "" : "s"}`}
                  >
              ...
              <ArrangementGrid
                audioRef={audioRef}
                indexedPlacements={timelineIndex.placements}
                isPlaying={isPlaying}
                onViewportWidthChange={setArrangementViewportWidth}
                onTimelineZoomChange={setTimelineZoom}
                onCommitPlacement={updatePlacementAtIndex}
                onPlaceSelectedClipAtBeat={placeSelectedClipAtBeat}
                onResetSelection={() => {
                  clearPlacementSelection();
                ...
                placementsByTrack={timelineIndex.placementsByTrack}
          ...
          <section className="workspace-row clip-row">
              ...
              <div className="list">
                {laneClips.map((clip) => (
                  <button
                    ...
                    onDrop={(event) => handleClipDrop(event, clip.id)}
                    onDragEnd={clearClipDragState}
                  >
                    <span
                    ...
                    <div className="clip-card-copy">
                      <div className="clip-card-title">
                        <span>{formatClipBarLength(clip.length_beats, timeline.beats_per_measure)}</span>
            ...
            <section className="panel clip-editor-panel">
              {selectedClip ? (
                <ShapeEditorPanel
                  onCommitShape={commitSelectedClipShape}
                      ...
                      <div className="clip-field-grid">
                        ...
                        <label className="clip-input-field field-span-2" title="Length / period">
                          ...
                          <select
                            ...
                            onChange={(event) => updateSelectedClipBeatValue(Number(event.target.value))}
                          >
...
function serializeVisualizer(config: TrackVisualizerConfig): string {
  return JSON.stringify(config);
}

function beatToPx(beat: number, timelineZoom: number): number {
  return Math.max(0, beat) * timelineZoom;
}

function pxToBeat(px: number, timelineZoom: number): number {
  return Math.max(0, px / timelineZoom);
}

function clampBeat(beat: number, totalTimelineBeats: number): number {
  return Math.max(0, Math.min(totalTimelineBeats, beat));
}

function snapBeatToGrid(beat: number, totalTimelineBeats: number): number {
  return Math.round(clampBeat(beat, totalTimelineBeats) * TIMELINE_SNAP_DIVISION) / TIMELINE_SNAP_DIVISION;
}

function snapPlaybackBeat(beat: number, totalTimelineBeats: number): number {
  return Math.round(clampBeat(beat, totalTimelineBeats) * PLAYHEAD_SNAP_DIVISION) / PLAYHEAD_SNAP_DIVISION;
}

function usePlaybackDisplayTime(
...
function pointerToPoint(svg: SVGSVGElement, clientX: number, clientY: number): LfoPoint {
  ...
  const phase = editorXToPhase(local.x);
  const value = editorYToValue(local.y);
  return {
    phase: snapLfoPointCoordinate(phase),
    value: snapLfoPointCoordinate(value),
  };
}
...
function clipListRangeLabel(clip: { source?: { kind: "lfo"; min: number; max: number } }): string {
  if (!clip.source?.kind) {
...
function formatClipBarLength(lengthBeats: number, beatsPerMeasure: number): string {
...
function ArrangementGrid({
  ...
  onSelectPlacement: (placementIndex: number, lane: LaneId, clipId: string, event: MouseEvent<HTMLDivElement>) => void;
  playbackTimeRef: { current: number };
              ...
              >
                {Array.from({ length: Math.ceil(totalTimelineBeats) + 1 }, (_, beat) => {
                  ...
                  return (
                    ...
                    >
                      {isBar && beat < totalTimelineBeats ? <span>{beat / timeline.beats_per_measure + 1}</span> : null}
                    </div>
                  );
                })}
              </div>
                ...
                <div key={lane} className="track-row">
                  ...
                  >
                    {lanePlacements.map((entry) => {
                      const placement = renderedPlacement(entry);
                      ...
                      const isActive = currentBeat >= placement.start_beat && currentBeat < placement.start_beat + entry.clip.length_beats * placement.repeats;
                      return (
                        <div
                          ...
                          onPointerDown={(event) => {
                            event.preventDefault();
                            event.stopPropagation();
                            onSelectPlacement(entry.placementIndex, lane, entry.clip.id, event);
                        ...
                        >
                          <span className="placement-label">
                            {formatPlacementLabel(entry.clip.name, width)}
                          </span>
...
function ShapeEditorPanel({
  ...
  function pointIndex(points: LfoPoint[], candidate: LfoPoint): number {
    return points.findIndex(
      (point) =>
        Math.abs(point.phase - candidate.phase) < 0.0001 &&
        Math.abs(point.value - candidate.value) < 0.0001,
    );
  }
          ...
          <div className="shape-editor-frame">
            <svg
              ...
              onPointerDown={(event) => {
                ...
                if (!svg) return;
                const role =
                  event.target instanceof SVGElement ? event.target.dataset.shapeRole : null;
...
function TimelineActionIcon({ name }: { name: "copy" | "delete" | "place" }) {
  const commonProps = {
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round" as const,
...
function TimelineToolIcon({ name }: { name: TimelineTool }) {
  const commonProps = {
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round" as const,
  ...
  return (
    <svg viewBox="0 0 20 20" aria-hidden="true">
      {name === "select" ? (
        <>
          <path d="M4.5 3.5 12 11H8.6l2.2 5-2 0.9-2.2-5L4.5 14Z" fill="currentColor" stroke="none" />
        </>
      ) : null}
      {name === "pencil" ? (
        <>
...
function TimelineFieldIcon({ name }: { name: "bpm" | "measures" | "time" }) {
  const commonProps = {
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round" as const,
...
function ClipFieldIcon({ name }: { name: "name" | "length" | "min" | "max" }) {
  const commonProps = {
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round" as const,
...
function ShapeModeIcon({ name }: { name: ShapeInteractionMode }) {
  const commonProps = {
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round" as const,
...
function PreviewCanvas({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      }, timelineIndexRef.current).uniforms;
...
function setUniform1f(gl: WebGL2RenderingContext, location: WebGLUniformLocation | null, value: number) {
  if (location != null) gl.uniform1f(location, value);
}

function setUniform2f(gl: WebGL2RenderingContext, location: WebGLUniformLocation | null, x: number, y: number) {
